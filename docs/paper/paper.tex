...
\item
  ...
  package. This method is described in detail in
...
Kruskal's algorithm
(\protect\hyperlink{ref-kruskal1956shortest}{Kruskal, 1956a}), and
others.
...
\begin{figure}
\hypertarget{fig:token-regions}{%
\centering
\includegraphics{figures/outputs-tokens-colored.tex}
\caption{Example text output format with token regions
highlighted.}\label{fig:token-regions}
}
\end{figure}
...
Algorithm}\label{success-rate-estimation}}
...
\begin{itemize}
\item
  \textbf{Shifted Sigmoid}: Creates sharp transition at \(x=0.5\)
  \[\sigma_s(x) = (1 + e^{-10^3 \cdot (x-0.5)})^{-1}\]
\item
  \textbf{h-function}: Transition function combining polynomial and
  sigmoid components
  \[h(q,a) = q \cdot (1 - |2q-1|^a) \cdot (1-\sigma_s(q)) + (1-(1-q) \cdot (1 - |2(1-q)-1|^a)) \cdot \sigma_s(q)\]
\item
  \textbf{Amplitude Scaling}: Weight-modulated polynomial
  \[A(q,a,w) = w \cdot (1 - |2q-1|^a)\]
\item
  \textbf{Soft Step}: Identity-like for \(p \approx 0.5\), pushes \(x\)
...
\begin{figure}
\centering
\includegraphics{figures/ep/ep_deadends_unique.pdf}
...
(\protect\hyperlink{ref-kruskal}{Kruskal, 1956b}), among others
...
Kruskal, J. B. (1956a). On the shortest spanning subtree of a graph and
...
Kruskal, J. B. (1956b). On the shortest spanning subtree of a graph and
the traveling salesman problem. \emph{Proceedings of the American
Mathematical Society}, \emph{7}(1), 48--50.
