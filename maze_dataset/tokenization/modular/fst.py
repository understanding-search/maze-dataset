...
def save_all_tokenizers_fst(verbose: bool = True, parallel: bool|int = False) -> FstSet:
	...
	all_tokenizers_names: list[str] = run_maybe_parallel(
		...
		pbar_kwargs=dict(
			total=n_tokenizers,
			desc="get name of each tokenizer",
			disable=not verbose
		),
	)

	assert n_tokenizers == len(all_tokenizers_names)
	print(f"# got {shorten_numerical_to_str(n_tokenizers)} ({n_tokenizers}) tokenizers names")
...
def check_tokenizers_fst(verbose: bool = True, parallel: bool|int = False) -> FstSet:
	...
	check_tokenizer_in_fst__do_except = functools.partial(check_tokenizer_in_fst, do_except=True)
